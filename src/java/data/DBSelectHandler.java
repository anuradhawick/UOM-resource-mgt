...
public class DBSelectHandler {
    ...
    public boolean isAvailable(String resourceID, Date startTime, Date endTime) throws SQLException {
        ...
        if (counter > 0) {
            ...
            PreparedStatement preparedStatement1 = connection.prepareStatement("SELECT count(resourceid) as count FROM resource_management.daily_time_slot WHERE resourceid=? and ((start_time <=? and end_time >=?) or (start_time <=? and end_time >=?)) and day=?;");
            ...
            preparedStatement1.setString(6, dayofweek);
